...
class Game:
	...
	def _identify_mines(self):
		self.mines = set()
		while len(self.mines) < self._num_mines:
			row = random.randint(0, self._num_rows - 1)
			column = random.randint(0, self._num_cols - 1)
	...
	def _create_cells(self):
	    ...
	    for i in range(self._num_rows):
	        for j in range(self._num_cols):
	        	self._cells[i][j].create_button(i, j, self._win)
	...
	def _identify_neighboring_mines(self, cell):
		...
		if cell._row == 0:
			...
			elif cell._col == self._num_cols - 1:
		...
		elif cell._row == self._num_rows - 1:
			if cell._col == 0:
		...
		#Left column edge cases
			...
			if cell._row > 0 and cell._row < self._num_rows - 1:
		...
		for neighbor in my_neighbors:
			...
			cell.neighbors.append(self._cells[i][j])
			if self._cells[i][j]._is_mine:
				cell.num_neighboring_mines += 1

	def uncover_mines(self):
		for mine in self.mines:
			i = mine[0]
			j = mine[1]
			self._cells[i][j].non_event_open()

	def you_lost_haha(self):
		self._win.stop_timer()
		self._check_flags()
		try_again = self._win.open_gif('images/nuke.gif', "Loser!")
		self._try_again_or_quit(try_again)

	def check_for_win(self):
		if self.num_cells_open == (self._num_rows * self._num_cols) - self._num_mines:
			self.you_won()
	...
	def _try_again_or_quit(self, try_again):
		if try_again:
	...
	def change_difficulty(self, difficulty):
		self.num_cells_open = 0
		self.num_cells_flagged = 0
		self.flagged_cells.head = None
		self.flagged_cells.tail = None
		self._difficulty = difficulty
		if self._difficulty == Difficulty.BEGINNER:
			self._num_rows = 9
			self._num_cols = 9
	...
	def update_flags(self):
		flags_left = self._num_mines - self.num_cells_flagged
	...
	def add_flag(self, flag_coordinates):
		self.flagged_cells.add_to_tail(Node(flag_coordinates))

	def del_flag(self, flag_coordinates):
		self.flagged_cells.remove(Node(flag_coordinates))

	def _check_flags(self):
		for flag in self.flagged_cells:
			if flag.val not in self.mines:
				i = flag.val[0]
				j = flag.val[1]
				self._cells[i][j].update_button_color("red")